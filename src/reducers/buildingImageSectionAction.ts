...
export const viewImageReducer = (
    state: BuildingImageState = buildingImageState,
    action: BuildingImageViewAction
) => {
    switch (action.type) {
        case ImageViewActionEnum.VIEW_IMAGE:
            const { idData, imageString, name }: BuildingImageState =
                getInformationFromArray(imageViewData, action.idData as number);
