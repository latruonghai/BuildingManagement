...
export const imageMethodReducer = (
    state: ImageDataState = initialStateForImage,
    action: ImageSlideShowAction
): ImageDataState => {
    switch (action.type) {
        case ImageHandleActionEnum.DELETE:
            return {
                ...state,
                imageDataArray: state.imageDataArray.filter(
                    (item, index) =>
                        index !== (action?.imageData?.orderInImage as number)
                )
            };
        case ImageHandleActionEnum.UPDATE:
            // delete action.type;
            // const number =
            state.imageDataArray[action.imageData!.orderInImage as number] =
                action.imageData as ImageSlideShowState;
            return state;
        case ImageHandleActionEnum.GET_ALL_IMAGES:
            ...
            return {
                ...state,
                imageDataArray: [
                    ...(action.imageDataArray as ImageSlideShowState[])
                ]
            };
