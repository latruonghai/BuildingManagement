...
import {
    SlideshowItemProps,
    DotSlideShowProps
} from '../types/components/SlideshowType';
        ...
        <Fragment>
            <div className="slideshow-container">
                ...
                <ButtonSlideshow
                    ...
                    classNameStyle="prev"
                    onClickHandler={() =>
                        dispatch(slideShowOrder(-1, numberOfSlideShowItem))
                    }
                />
                <ButtonSlideshow
                    contentButton="&#10095;"
            ...
            <br />
            <DotSlideShow
                numberItems={numberOfSlideShowItem}
                currentItemDot={index}
                onClickHandler={dispatch}></DotSlideShow>
        </Fragment>
    );
};
...
const ButtonSlideshow = (props: ButtonProps) => {
    return (
        <Fragment>
            <a className={props.classNameStyle} onClick={props.onClickHandler}>
                {props.contentButton}
            </a>
        </Fragment>
    );
    ...
    return (
        <Fragment>
            <div className={'slideshow-item fade' + props?.class_name}>
                <img
                    src={props.imSrc}
                    alt="Apartment Information"
                    className="hover:cursor-pointer"
                    title={props.title}></img>
                <h4>{props.title}</h4>
                <div className="slideshow-item-text"></div>
            </div>
        </Fragment>
    );
};
const DotSlideShow = (props: DotSlideShowProps) => {
        ...
        spanItem.push(
            i !== props.currentItemDot ? (
                <span
                    id={`dot-${i}`}
                    key={i}
                    className="slideshow-dot-item"
                    onClick={handleSelectedDot}></span>
            ) : (
                <span
                    id={`dot-${i}`}
                    key={i}
                    className="slideshow-dot-item active-item"
                    onClick={handleSelectedDot}></span>
            )
        );
    }
    ...
    return (
        <Fragment>
            <div className="slideshow-dot">{spanItem}</div>
        </Fragment>
    );
};
