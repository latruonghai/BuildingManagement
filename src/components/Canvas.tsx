...
const Canvas = ({ isShowing, imgData, toggle }: CanvasProps): JSX.Element => {
    const canvasRef: MutableRefObject<HTMLCanvasElement> =
        useRef<HTMLCanvasElement>(null) as MutableRefObject<HTMLCanvasElement>;
    const imgSrc = imgData?.imSrc as string;

    const ctxRef: MutableRefObject<CanvasRenderingContext2D> =
        useRef<CanvasRenderingContext2D>(
            null
        ) as MutableRefObject<CanvasRenderingContext2D>;
    const stateCanvas: StateCanvas = useSelector(
                    ...
                    <div className="drawing-section overflow-auto">
                        ...
                        <div className="w-full h-auto rounded-sm mt-3">
                            <canvas
                                ...
                                onMouseMove={draw}
                                onLoad={handleClick}></canvas>
                        </div>
                    </div>
                    <div className="button-area">
                        <Button
                            classNameStyle="font-medium mb-2"
                            contentButton="Add Image"
                            onClickHandler={handleClick}
                        />
                        <Button
                            classNameStyle="font-medium mb-2"
                            contentButton="Update Image"
                            onClickHandler={() => {
                        ...
                        />
                        <Button
                            classNameStyle="font-medium mb-2 ml-2"
                            contentButton="Exit"
                            onClickHandler={() =>
                                dispatch(
                                    canvasActions(
                                        CanvasActionEnum.SET_IS_SHOWING,
                                        false
                                    )
                                )
                            }
            ...
            <div className="menu">
                ...
                <input
                    ...
                    onChange={(ev) =>
                        dispatch!(
                            canvasActions(
                                CanvasActionEnum.SET_LINE_COLOR,
                                ev.target.value
                            )
                        )
                    }
                ...
                <input
                    ...
                    onChange={(ev) =>
                        dispatch!(
                            canvasActions(
                                CanvasActionEnum.SET_LINE_WIDTH,
                                ev.target.value
                            )
                        )
                    }
