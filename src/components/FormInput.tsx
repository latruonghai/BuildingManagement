...
const FormInput = (props: FormInputProps) => {
    ...
    const inputState = useSelector(
        (state: RootState) => state.onChangeInputReducer
    );
    const onChangeHandler = (ev: any) => {
        ...
        dispatch(
            onChangeInputField(InputFormActionEnum.ON_CHANGE, {
                value: value,
                name: name
            })
        );
    ...
    const button = (
        <Fragment>
            <Button
                contentButton={'Close'}
                classNameStyle="close"
                onClickHandler={() => {
                    // fetch
                    FetchAllBuilding(dispatch);
                    dispatch(
                        toggleFormAction(ToggleFormActionEnum.UNTOGGLE_FORM)
                    );
                }}
            />
            <Button
                contentButton=" Save"
                classNameStyle="fas fa-save"
                onClickHandler={() => {
    ...
    return (
        <Fragment>
            <Form
                title={props.title}
                isFormShowing={props.isFormShowing}
                footerSectionComponent={button}>
                <InputSection
                        ...
                        <div className="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                            <label
                                ...
                                <Input
                                    ...
                                    type={item.type}
                                    name={item.name}
                                    handleCallback={props.onChangeCallBack}
                                />
                            </label>
                        </div>
                    );
