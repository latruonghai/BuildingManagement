...
import {
    onDelTableDataClickHandler,
    onEditTableDataClickHandler
...
const Table = (props: any) => {
    ...
    const tableData: TableData = tableProperties(
        imageSlideShowState.imageDataArray
    );
    //
                    ...
                    <tbody>
                        {tableData!.body!.map(
                            (item: ImageSlideShowState, index: number) => {
                                return (
                                    <Row body={item} id_name={`row-${index}`} />
                                );
                            }
                        )}
                    </tbody>
    ...
    return (
        <Fragment>
            <tr id={id_name} className="rows-section !border-2">
                <th className="cell header">{body.id}</th>
                <td className="cell ">{body.title}</td>
                <td className="cell">
                    <img
                        className="h-4/5"
                        alt=""
                ...
                <td>
                    <Button
                        classNameStyle="fas fa-edit edit"
                        onClickHandler={onEditTableDataClickHandler(
                            dispatch,
                            imageSlideShowState
                        )}
                        id_name={`button-${id_name}`}
                        contentButton={' Edit'}></Button>
                    <Button
                        onClickHandler={onDelTableDataClickHandler(
                            dispatch,
                            imageSlideShowState,
                            idSelector as number
                        )}
                        classNameStyle="fas fa-trash-alt delete"
                        id_name={`button-${id_name}`}
                        contentButton={' Delete'}></Button>
                </td>
            </tr>
        </Fragment>
    );
