...
export const onDelTableDataClickHandler = (dispatch: Dispatch<any>, imageSlideShowState: ImageDataState, selectorId: number): Void => {

    return (ev: any) => {
        // const id = ev.target.id;
        const index = getIndexFromIdName(ev.target.id);

        const image_index = imageSlideShowState.imageDataArray[index as number].id;
