...
export const getInformationFromArray = (
    array: ImageSectionProps[],
    index: number
): BuildingImageState => {
...
export const getImageSlideShowStateDataAction = (
...
export const getImageSlideShowApartmentData = (
    imageDataArray: any
): ImageSlideShowState[] => {
    ...
    for (let imageData of imageDataArray) {
        imageDataState.push({
            imSrc: _.startsWith(imageData.ImagePath, '/')
                ? API_URL + imageData.ImagePath
                : imageData.ImagePath,
            title: imageData.name,
            id: imageData.id,
